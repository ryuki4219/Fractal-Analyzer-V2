# Fractal Analyzer V2 - システムアーキテクチャ
...
Fractal Analyzer V2は、フラクタル次元解析とAI予測を組み合わせた肌品質評価システムです。

**主要機能:**
- 📊 **学習モード**: 高画質画像からAIモデルを訓練
- 🔮 **推論モード**: 低画質画像のFD値をAI補正
- 🧪 **肌品質評価**: フラクタル解析による肌状態の定量評価
- ✅ **検証モード**: モデル精度の検証とレポート生成
...
Fractal-Analyzer-V2/
├── 📄 コアモジュール (3ファイル)
│   ├── fractal_app.py                 # メインアプリケーション (5,719行)
│   ├── image_quality_assessor.py      # 画質自動判定 (610行)
│   └── skin_quality_evaluator.py      # 肌品質評価 (388行)
│
├── 🎨 ビューアーツール (3ファイル)
│   ├── image_viewer.py                # 汎用画像ビューアー
│   ├── skin_data_viewer.py            # 肌データビューアー
│   └── skin_viewer.py                 # 肌画像ビューアー
│
├── 💾 データ・モデル
│   ├── trained_fd_model.pkl           # 学習済みLightGBMモデル
...
│   ├── VALIDATION_MODE_GUIDE.md       # 検証モードガイド
│   ├── QUALITY_THRESHOLDS_GUIDE.md    # 品質閾値ガイド
│   ├── MODEL_MANAGEMENT.md            # モデル管理
...
└── 🚀 起動スクリプト (4ファイル)
    ├── 起動.bat                       # 標準起動
    ├── 簡単起動.bat                   # 簡易起動
    ├── トラブルシューティング.bat      # トラブルシュート
    └── デスクトップショートカット作成.bat
```

**総ファイル数: 約30ファイル** (開発ファイル削除後)
...
### 1️⃣ メインアプリケーション (fractal_app.py)
